...
import com.chirag.busroutes.RoutesAdapter.ClickHandler;
...
public class MainActivity extends AppCompatActivity implements OnClickListener {
    ...
    private PlaceSelectionListener getDestPlaceSelectionListener() {
        return new PlaceSelectionListener() {
            ...
            public void onPlaceSelected(Place place) {
                ...
                googleMap.addMarker(new MarkerOptions().position(destLatLong).title("Destination").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_RED))).showInfoWindow();
                googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(destLatLong, 12));
            }

            @Override
            public void onError(Status status) {
                Log.d(TAG, "An error occurred during dest: " + status);
            }
        };
    }
    ...
    private PlaceSelectionListener getSourcePlaceSelectedListener() {
        return new PlaceSelectionListener() {
            ...
            public void onPlaceSelected(Place place) {
                ...
                googleMap.addMarker(new MarkerOptions().position(sourceLatLong).title("Source").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_BLUE))).showInfoWindow();
                googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(sourceLatLong, 12));
            }

            @Override
            public void onError(Status status) {
                Log.d(TAG, "An error occurred during source: " + status);
            }
        };
    }
    ...
    private void search() {
        ...
        String url = baseUrl + "json?mode=transit&alternatives=true&key=[YOUR_KEY-HERE]" + "&origin=place_id:" + placeIdSource + "&destination=place_id:" + placeIdDest;
        Log.d(TAG, url);
    ...
    private void handleTransitApiResponse(TransitApiResponse transitApiResponse) {
        //clear the list to avoid excessive display
        stepsToBeDisplayedInPopUp.clear();
        ...
        //counter case to check availability of buses
        if(stepsToBeDisplayedInPopUp.size()!=0){
            showPopUp(stepsToBeDisplayedInPopUp);
        }else{
            Log.d(TAG,"No available transit routes");
            showToast("Sorry, No Buses available between given locations");
        }
    ...
    private void showPopUp(List<Step> stepsToBeDisplayedInPopUp) {
        ...
        //clearing the previously drawn polylines
        if(line!=null)
        {
            line.remove();
        }
        ...
        return new ClickHandler() {
            ...
            public OnClickListener onItemClick(final Step step) {
                return new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        showToast(step.getTransitDetails().getName() + " - " + step.getTransitDetails().getShortName());
                        List<LatLng> list = decodePoly(step.getPolyLinePoints());
                        line = googleMap.addPolyline(new PolylineOptions().addAll(list).color(Color.BLUE));
                    }
                };
            }
        };
